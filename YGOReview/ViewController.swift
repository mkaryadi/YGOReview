...
class ViewController: UIViewController {
  ...
  @IBAction func Sign_Up(_ sender: Any) {
    if password.isEmpty || email.isEmpty || email.count < 6 || password.count < 6 {
      print("ABDIWAHID PAssword is", password)
      print("DEBUG", "Email is", email)
      displayAlert(
        "Invalid Email Password", "Please Enter a valid email/Password",
        "An email Password error")
    } else if password.isEmpty
      || password.count < 6 && !email.isEmpty && email.count > 6 && email.contains("@")
    {
      displayAlert(
        "PASSWORD ERROR", "Please Enter a valid password" + " 6 digits",
        "An Password error")
    } else if email.isEmpty || email.count < 6
      || !email.contains("@") && !password.isEmpty && password.count > 6
    {

      displayAlert(
        "Email ERROR", "Please Enter a valid email" + " with @ also",
        "An password error")
    } else {
      FirebaseAuth.Auth.auth().createUser(
        ...
        completion: { result, error in
          guard error == nil else {
            self.displayAlert(
              "Firebase Login Issue", "Cant login you in try again later",
          ...
          let Logined_In_User =
            storyboard.instantiateViewController(withIdentifier: "Sucess") as! Signed_In
      ...
      || !email.contains("@") && !password.isEmpty && password.count > 6
    {
      ...
      FirebaseAuth.Auth.auth().signIn(
        ...
        completion: { result, error in
          guard error == nil else {
            self.displayAlert(
              "Firebase Login Issue", "Cant Sign you in try again later",
              "FireBase login Issues")
            return
          }
